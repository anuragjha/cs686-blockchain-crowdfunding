...
func EncryptMessageWithPublicKey(hash1 hash.Hash, publicKey *rsa.PublicKey, message string, label string) []byte {

	//encrypted, err := rsa.EncryptOAEP(hash1, rand.Reader, publicKey, []byte(message), []byte(label))
...
func DecryptMessageWithPrivateKey(hash1 hash.Hash, privateKey *rsa.PrivateKey, encrypted string, label string) []byte {
	//decrypted, err := rsa.DecryptOAEP(hash1, rand.Reader, privateKey, []byte(encrypted), []byte(label))
	decrypted, err := rsa.DecryptPKCS1v15(rand.Reader, privateKey, []byte(encrypted))
	if err != nil {
